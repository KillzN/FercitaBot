...
export async function handler(chatUpdate) {
  ...
  /* Creditos a Otosaka (https://wa.me/56937835229) */
    ...
    case 'remove':
      if (chat.welcome && !chat?.isBanned) {
        ...
        for (const user of participants) {
          let pp = 'https://raw.githubusercontent.com/Fercita/FercitaBot/master/src/avatar_contact.png';
          try {
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;Fercita 👑;;;\nFN:Fercita 👑\nORG:Fercita 👑\nTITLE:\nitem1.TEL;waid=56937835229:+51 992 621 601\nitem1.X-ABLabel:Fercita 👑\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:Fercita 👑\nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, { contacts: { displayName: 'Fercita 👑', contacts: [{ vcard }] } }, { quoted: callmsg });
